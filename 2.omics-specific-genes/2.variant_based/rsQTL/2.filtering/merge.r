library(dplyr)
library(ggplot2)
eqtl<-read.table('eqtl.nominal.qvalue.txt.rsQTL',head=T,sep="\t")
rqtl<-read.table('rqtl.nominal.qvalue.txt.rsQTL',head=T,sep="\t")
pqtl<-read.table('pqtl.nominal.qvalue.txt.rsQTL',head=T,sep="\t")
...
p<-ggplot(pqtl,aes(x=-log10(P.nominal)))+
   ...
   theme_bw()
ggsave('rsqtl.pqtl.nominalP.hist.png',p)
...
ggsave('rsqtl.nominalP.hist.png',p)
...
head(data)
write.table(data,'rsqtl.with_other_omics.txt',quote=F,sep="\t",row.names=F,col.names=T)
