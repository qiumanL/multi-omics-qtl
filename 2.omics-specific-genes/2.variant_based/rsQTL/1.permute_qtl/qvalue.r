...
write.table(dd.sig,"rqtl.permute.qvalue.rsqtl.txt", quote=F, row.names=F, col.names=T, sep="\t")

dd.shared<- dd[dd$qvalue>=0.1,]
write.table(dd.shared,"rqtl.permute.qvalue.shared.txt", quote=F, row.names=F, col.names=T, sep="\t")
