...
observed<-as.matrix(read.table(paste0("1.QTL/shared_sample_shared_genes/",typeObs,"/expression/sub.expr.gene"),head=T,row.names=1,sep="\t",check.names=F))
predicted<-list()
for(type in typelist){
    predicted[[type]]<-as.matrix(read.table(paste0("1.QTL/shared_sample_shared_genes/",type,"/expression/sub.expr.gene"),head=T,row.names=1,sep="\t",check.names=F))
...
# rna / ribo / protein
genelist<-rownames(read.table("1.QTL/shared_sample_shared_genes/eQTL/expression/sub.expr.gene",head=T,row.names=1,sep="\t"))
