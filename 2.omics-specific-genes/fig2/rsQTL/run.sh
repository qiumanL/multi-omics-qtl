## chr=12 gene=ENSG00000102595 name=UGGT2
chr=$1
gene=$2
name=$3
mkdir $gene
...
### coloc snp
top_coloc_snp=`awk -F "\t" 'NR>1{print $1}' <(head -2 /vg_sklmgdata_hw_01/data/liangqiuman/psychencode/another_analysis/6.coloc/rqtl/chr"$chr"/"$gene"/coloc_snp_PP4.txt)`
echo $top_coloc_snp >top_coloc_snp

############## for boxplot
top_rqtl_snp=`awk '$1=="'$gene'"{print $8}' /vg_sklmgdata_hw_01/data/liangqiuman/psychencode/another_analysis/1.qtl/shared/rqtl/rqtl.permute.qvalue.txt`
echo $top_rqtl_snp >top_rqtl_snp
...
zgrep "$top_rqtl_snp" /vg_sklmgdata_hw_01/data/liangqiuman/psychencode/another_analysis/1.qtl/shared/genotype/genotypes.all.chr"$chr".realign.vcf.gz|awk -F "\t" '{for(i=9;i<=NF;i++){printf $i"\n"}}' >genotype
