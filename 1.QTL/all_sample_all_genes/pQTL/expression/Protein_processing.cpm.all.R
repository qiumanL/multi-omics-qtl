...
meta<-read.table("/zs32/data-analysis/liucy_group/jiangyi/psychENCODE/meta/synapse-meta-clinical-technical-data-BrainGVEX-RNAseq-TonyaCurrated-V2-GGedits3.txt",sep="\t",head=T)
...
#normadj <- (0.5+0.5*cor(log2prot.fgene))^2
...
# use bicor: 22 outlier samples (log2prot-based), 24 outlier samples (counts-based), 23 outlier samples (tpm-based)
...
rMeans<-rowMeans(log2prot.fgene.fsample)
rSDs<-apply(log2prot.fgene.fsample,1,sd)
log2prot.fgene.fsample.norm<-apply(log2prot.fgene.fsample,2,function(x) (x-rMeans)/rSDs)
write.table(log2prot.fgene.fsample.norm, file="log2prot.fgene.fsample.norm", sep="\t", row.names=T, quote=F,col.names=NA)
...
log2prot.fgene.fsample.norm.qn<-normalize.quantiles(as.matrix(log2prot.fgene.fsample.norm),copy=T)  # Quantile normalization across columns
rownames(log2prot.fgene.fsample.norm.qn)<-rownames(log2prot.fgene.fsample.norm)
colnames(log2prot.fgene.fsample.norm.qn)<-colnames(log2prot.fgene.fsample.norm)
write.table(log2prot.fgene.fsample.norm.qn, file="log2prot.fgene.fsample.norm.qn", sep="\t", row.names=T, quote=F,col.names=NA)
...
library(reshape)
library(ggplot2)
log2prot.fgene.fsample.norm.melt<-melt(log2prot.fgene.fsample.norm)
log2prot.fgene.fsample.qn.melt<-melt(log2prot.fgene.fsample.qn)
pdf("distribution.log2prot.fgene.fsample.norm.pdf")
p<-ggplot(log2prot.fgene.fsample.norm.melt)+geom_density(aes(x=value,group=X2),color=rgb(0.2,0.2,0.2,0.2))+theme(legend.position = "none")
print(p)
dev.off()
...
## realign sample ID for atac-seq sequencing batch information
nFactor<-19
