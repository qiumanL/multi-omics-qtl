...
time awk 'NR==FNR&&FNR>1{a[$1]++}NR>FNR&&!b[$1]++&&($1 in a){print $1}' Brain_Frontal_Cortex_BA9.allpairs.txt.format <(zcat ../eqtl/eqtl.cpm.all.peerFactor30/eqtl.nominal.allpair.chr{1..22}.gz) > share.gene &
time awk 'NR==FNR&&FNR>1{a[$2]++}NR>FNR{k=$9"_"$10;if(!b[k]++&&(k in a))print k}' Brain_Frontal_Cortex_BA9.allpairs.txt.format <(zcat ../eqtl/eqtl.cpm.all.peerFactor30/eqtl.nominal.allpair.chr{1..22}.gz) > share.snp &
...
time awk 'ARGIND==1{g[$1]++}ARGIND==2{s[$1]++}ARGIND>=3{k=$9"_"$10;if(($1 in g)&&(k in s)){print $1"\t"k"\t"$12"\t"$14}}' share.gene share.snp <(zcat ../eqtl/eqtl.cpm.all.peerFactor30/eqtl.nominal.allpair.chr{1..22}.gz) |awk '!a[$1$2]++' > eqtl.braingvex.nominal.chrall &
