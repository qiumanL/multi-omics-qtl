ln -s log2prot.fgene.fsample.qn.lmPEER19.norm.sub.prot2gene origin.expr
...
awk 'BEGIN{FS="\t";OFS="\t"}NR==FNR&&$5=="gene"{split($8,s,".");a[s[1]]=$1"\t"$2-1"\t"$3"\t"s[1]"\t"s[1]"\t"$6}NR>FNR&&FNR==1{$1="#chr\tstart\tend\tgene\tgene\tstrand";print $0}NR>FNR&&FNR>1&&($1 in a){$1=a[$1];print $0}' <(zcat gencode.v19.annotation.gz) sub.expr |sed -e's/^chr//' > sub.expr.bed  # convert to bed
