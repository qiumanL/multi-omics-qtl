...
spec <- matrix(
  ...
  "protein", "p", 2, "character","This is protein id!"),
  byrow=TRUE, ncol=5)
opt <- getopt(spec=spec)
#### LD_clumped snplist
pqtl_snplist<-read.table(paste0('/vg_sklmgdata_hw_01/data/liangqiuman/psychencode/another_analysis/code/4.MR/prepare_files/susie_fine-mapping/pQTL/chr',opt$chr,"/",opt$gene,"/",opt$protein,'.cs.snp.txt'))[,1]
pqtl_snplist
###### read our own data
...
res<-mr(dat)
res
write.table(res,paste0(opt$protein,'.twosampleMR.result.txt'),quote=F,sep="\t",col.names=T,row.names=F)
...
output
write.table(output,paste0(opt$protein,'.twosampleMR.egger.result.txt'),quote=F,sep="\t",col.names=T,row.names=F)
